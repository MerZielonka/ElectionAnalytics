...
My model formula looked like the following, and I swapped out the party and dataset accordingly, ending up with two different types of model and one set for each party.
...
The coefficient of president's party reflects the concept of incumbent presidents' parties being punished, particularly during midterms. In coding the variable, I designated +1 to Democratic presidents and -1 to Republican presidents. This reflects that if the president is a Democrat, the party is expected to lose 15 seats. In midterm years, this is exacerbated to 24 seats. 
...
To evaluate my model, there are two types of testing we can conduct, in-sample fit and out-of-sample fit. In-sample fit uses the data we already have through looking at the r-squareds and comparing the in-sample error, or how often a prediction on historical data matches the historical result. I examined the r-squareds of my model and found that they are relatively strong, ranging around .86. To compare the in-sample error, I graphed historical results versus predictions.
...
```
Based on the in-sample error graphs, the models based on more data points, i.e. all the elections instead of just midterms, seem to hit more historical data points and are more trustworthy. Finally, I calculated the mean squared error for each model. They all ended up around 10, so the models should be relatively comparable.

## Out-of-Sample Fit

In order to conduct out-of-sample fit tests, we withhold some observation then try to predict its result. 
...
```
The out-of-sample testing reveals that on Democrats, the models underpredict the results and on Republicans, the models overpredict the results. I suspect this has to do with the president's party variable. This is something to consider when it comes time to decide which model to choose for my final prediction.
...
```
These are relatively wide intervals of prediction, proving that predicting elections is really not a science. 

# Final Prediction and Conclusion

Based on all of the investigations I have just completed regarding the accuracy of my models, I've decided to use the models based on all of the historical election data, not just midterms. Not only does that model have a smaller interval of prediction but it performed better on the out-of-sample fit test. While the adjusted r-squareds were also higher, it was by such a small amount (0.003) that it seems insignificant. They also had marginally smaller residual standard errors. Overall, I believe that the predictions that have more training data are more reliable. Using the fitted values, I predict that *the Democratic party will win 205 seats and the Republican party will win 225 seats in the 2022 midterms.* 
